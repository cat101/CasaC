 ...
 * a series of variable length mark/space signals.  Depending on the protocol they vary the links of the 
...
void IRsendBase::sendGeneric(unsigned long data, int Num_Bits, int Head_Mark, int Head_Space, int Mark_One, int Mark_Zero, int Space_One, int Space_Zero, int kHz, bool Use_Stop) {
 ...
 */
void IRsendRaw::send(unsigned int buf[], int len, int khz)
{
  enableIROut(khz);
  for (int i = 0; i < len; i++) {
    if (i & 1) {
  ...
  space(0); // Just to be sure we turn off the IR
}
/*
...
/*
 * Used for Daikin AC. Taken from https://github.com/mharizanov/Daikin-AC-remote-control-over-the-Internet
      TODO: This function could replace sendGeneric since it is more general.
 */
 void IRsendDaikin::send(unsigned char buf[], int len, int start) {
  int data2;
  enableIROut(38);
...
#define AIWA_HDR_MARK     9137 
#define AIWA_HDR_SPACE  4449 
#define AIWA_ONE_MARK     637
#define AIWA_ONE_SPACE  1624 
#define AIWA_ZERO_MARK  637
#define AIWA_ZERO_SPACE   493

/* 
  These routine was tested with a RC-TN340EX remote
  repeat starts 22000 after command and then looks like
    mark(9134);
    space(4454); 
    // Stop bit
    mark(AIWA_ONE_MARK);
    space(0); // Just to be sure we turn off the IR
*/
void IRsendAIWA::send(unsigned int data) {
  unsigned long data_pre=(0x1D8113FUL<<6);  //Shift to left to complete a 32 bit int (send only 26 bits)
  enableIROut(38);
  //Header
  mark(AIWA_HDR_MARK);
  space(AIWA_HDR_SPACE);
  //Send pre data
  for (int i = 0; i < 26; i++) {
    if (data_pre & TOPBIT) {
      mark(AIWA_ONE_MARK); space(AIWA_ONE_SPACE);
    } 
    else {
      mark(AIWA_ZERO_MARK); space(AIWA_ZERO_SPACE);
    }
    data_pre <<= 1;
  }
  //Send code
...
// Some protocols need to do custom header work.
  ...
  if (Head_Mark) {if (!MATCH_MARK(rawbuf[1],Head_Mark))    {Serial.println("HEADER_MARK_ERROR");return HEADER_MARK_ERROR;}}
  if (Head_Space) {if (!MATCH_SPACE(rawbuf[2],Head_Space)) {Serial.println("HEADER_SPACE_ERROR");return HEADER_SPACE_ERROR;}}

  if (Mark_One) {//Length of a mark indicates data "0" or "1". Space_Zero is ignored.
    offset=2;//skip initial gap plus header Mark.
    Max=rawlen;
    while (offset < Max) {
      if (!MATCH_SPACE(rawbuf[offset], Space_One)) {Serial.println("DATA_SPACE_ERROR");return DATA_SPACE_ERROR;}
      offset++;
      ...
      else {Serial.print(offset);Serial.println(" DATA_SPACE_ERROR");return DATA_SPACE_ERROR;}
      offset++;
    }
    bits = (offset - 1) / 2;
  }
    ...
    while (offset < Max) {
      if (!MATCH_MARK (rawbuf[offset],Mark_Zero)) {Serial.println("DATA_MARK_ERROR");return DATA_MARK_ERROR;}
      offset++;
      ...
      else {Serial.println("DATA_SPACE_ERROR");return DATA_SPACE_ERROR;}
      offset++;
    }
