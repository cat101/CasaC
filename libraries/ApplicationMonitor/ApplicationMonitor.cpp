...
void CApplicationMonitor::ResetArduino()
{
  ...
  wdt_enable(WDTO_120MS);  //Changing the WDT period also disables the interrup and therefore this won't be logged as a crash
  while (true)
    ;
}
...
void CApplicationMonitor::Dump(void) const
{
  ...
  LoadHeader(Header);
  DEBUG_PRINT_P("CApplicationMonitor %d %d\n", Header.m_uSavedReports, Header.m_uNextReport);

  if (Header.m_uSavedReports != 0){
    LoadReport(0, Report);
    uAddress = 0;
    memcpy(&uAddress, Report.m_auAddress, PROGRAM_COUNTER_SIZE);
    logException_P("Crash %04X",uAddress); //Print the word address where we crashed
    //Clear the header so we don't report on next boot
...
void __attribute__ ((noinline)) CApplicationMonitor::WatchdogInterruptHandler(uint8_t *puProgramAddress)
{
  ...
  //Only save one report
  Header.m_uNextReport = 0;
  Header.m_uSavedReports = 1;
